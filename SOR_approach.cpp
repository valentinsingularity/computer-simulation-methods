...
{
	int i,j,n;
	double V[12][22],previous_V[12][22],change,w;
	ofstream fout("output.txt");
	
	for(i=0;i<=11;i++)
	  for(j=0;j<=21;j++)
	    if(i==0 || j==0 || i==11 || j==21) V[i][j]=1;
		else V[i][j]= 0.7 * exact_form_V_5(j,i); 
		
	for(i=0;i<=11;i++)
	  for(j=0;j<=21;j++)
	    previous_V[i][j]=V[i][j];
		
	w=2/(1+M_PI/15);	// L ~ (L_x+L_y)/2 = 15; w~1.65, 1<w<2	
    ...
    for(n=1;;n++)
    {
    	...
    	for(i=1;i<=10;i++)
	      for(j=1;j<=20;j++)
	      {
	         V[i][j] = (1-w)*previous_V[i][j] + (w/4)*(previous_V[i+1][j]+V[i-1][j]+previous_V[i][j+1]+V[i][j-1]);
